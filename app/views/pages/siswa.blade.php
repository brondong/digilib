		...
		<span class="control-group" id="control-urut-siswa">
			...
			<span class="controls">
				{{ Form::select('kolom-siswa', array(
					...
					), 'nis', array('id' => 'kolom-siswa', 'onchange' =>'urutSiswa()'));
				}}
			</span>
		</span>
			<span class="controls">
				{{ Form::select('tipe-siswa', array('asc' => 'Ascending', 'desc' => 'Descending'), 'asc', array('id' => 'tipe-siswa', 'onchange' =>'urutSiswa()')); }}
			</span>
...
@else
	<div class="alert kosong">
		<strong>Peringatan!</strong> Belum ada data siswa yang tersimpan di basis data.
	</div>

	<div id="jarak"></div>

	<div class="btn-group">
		{{ Form::button('<i class="icon-plus"></i>', array('class' => 'btn', 'onclick' => 'modalTambahSiswa()')) }}
	</div>
@endif
</div>