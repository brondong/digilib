...
$excel->getProperties()
	...
	->setSubject('Data Pengembalian')
	->setDescription('Data Pengembalian ' . $sekolah->nama);
...
$sheet
	...
	->setCellValue('A2', 'DATA PENGEMBALIAN')
...
$sheet->getStyle('A6:F' . (count($kembali) + 5))->applyFromArray($style);
$sheet->getStyle('A6:F' . (count($kembali) + 5))->getAlignment()->setWrapText(true);
$sheet->getStyle('A6:F' . (count($kembali) + 5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$sheet->getStyle('B6:C' . (count($kembali) + 5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
$sheet->getStyle('D6:D' . (count($kembali) + 5))->getNumberFormat()->setFormatCode('#,##0');
